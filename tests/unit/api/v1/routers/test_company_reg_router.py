...
from tests.utils import list_prepare, prepare_payload
...
class TestCompanyRegRouter:
    ...
    async def test_check_account(
        ...
        with expectation:
            ...
            response = await async_client.get(
                url, params={"account_email": account_email}, headers=headers
            )
    ...
    async def test_get_all_companies(
        ...
        with expectation:
            ...
            assert response.status_code == expected_status_code
            assert (
                list_prepare(response, ["id", "created_at", "updated_at"])
                == expected_payload
            )
