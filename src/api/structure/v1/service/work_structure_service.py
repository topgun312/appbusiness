...
class WorkStructureService(BaseService):
    ...
    async def create_department(
        self, company_name: str, struct_adm_data: dict
    ) -> StructAdmDB:
        """Create department of company"""
        struct_adm_name, struct_adm_parent = (
            struct_adm_data["name"],
            struct_adm_data["parent"],
        )
        ...
        if struct_adm_name == company_name:
            ...
            try:
                struct_adm: StructAdmModel = (
                    await self.uow.struct_adm.add_one_and_get_obj(
                ...
                self._incorrect_parent_or_name_exists_error()
        parent_struct_adm: Ltree | None = (
            await self.uow.struct_adm.get_all_path_of_parent(struct_adm_parent)
        )
    ...
    @transaction_mode
    async def update_department_by_name(
        self, struct_adm_name: str, department_data: dict
    ) -> StructAdmDB:
        """Update department of company by name"""
        children: Sequence[
            Row[tuple[Any, ...] | Any]
        ] = await self.uow.struct_adm.get_children_paths(
            struct_adm_name=struct_adm_name
        )
        department: StructAdmModel = await self.uow.struct_adm.get_by_query_one_or_none(
            name=struct_adm_name
        )
        ...
        updated_department: StructAdmModel = (
            await self.uow.struct_adm.update_one_by_name(
                _name=struct_adm_name, name=dep_data_name, path=Ltree(new_path)
            )
        )
    ...
    @transaction_mode
    async def update_position_by_name(
        self, position_name: str, position_data: dict
    ) -> PositionDB:
        ...
        updated_position: PositionModel = await self.uow.position.update_one_by_name(
            _name=position_name, **position_data
        )
    ...
    async def add_users_to_position(
        ...
        for user_id in users_position_data["user_id"]:
            ...
            user_position: UserPositionModel = (
                await self.uow.user_position.add_one_and_get_obj(
    ...
    async def add_position_to_department(
        self, struct_adm_pos_data: dict
    ) -> StructAdmPositionDB:
        """Add position to department"""
        struct_adm: StructAdmModel = await self.uow.struct_adm.get_by_query_one_or_none(
            id=struct_adm_pos_data["struct_adm_id"]
        )
        self._check_department_exists(department=struct_adm)
        position: PositionModel = await self.uow.position.get_by_query_one_or_none(
            id=struct_adm_pos_data["position_id"]
        )
        self._check_position_exists(position=position)
        struct_adm_pos: StructAdmPositionModel = (
            await self.uow.struct_adm_position.add_one_and_get_obj(
                struct_adm_id=struct_adm.id, position_id=position.id
            )
        )
    ...
    async def add_department_head(
        self, user_id: UUID4, struct_adm_name: str
    ) -> StructAdmDB:
        """Add department head"""
        user: User = await self.uow.user.get_by_query_one_or_none(id=user_id)
        self._check_user_exists(user=user)
        ...
        self._check_department_exists(department=struct_adm)
        try:
            struct_adm_head: StructAdmModel = (
                await self.uow.struct_adm.update_one_by_name(
                    _name=struct_adm_name, head_user_id=user.id
                )
            )
    ...
    def _incorrect_head_exists_error() -> None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="The user is already the head of another department",
        )
