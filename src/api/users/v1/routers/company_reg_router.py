...
async def sign_up(
    ...
    return {
        "status": status.HTTP_200_OK,
        "detail": "The invite_code and email are valid! Please complete the registration of the company",
    }
...
async def update_company_by_id(
    ...
    if admin:
        updated_company: CompanyDB = await service.update_company_by_id(
            company_id=company_id, company_data=company_data.model_dump()
        )
