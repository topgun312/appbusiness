...
class CompanyRegService(BaseService):
    ...
    async def check_account(
        ...
        new_account: AccountModel = await self.uow.account.add_one_and_get_obj(
            email=email
        )
    ...
    async def sign_up_complete(
        ...
        await self.uow.secret.add_one(
            user_id=user_id,
            account_id=account.id,
            password=await auth_utils.hash_password(secret_data.get("password")),
        )
    ...
    async def get_all_companies(self, **kwargs) -> Sequence[CompanyDB]:
        ...
        companies: Sequence[CompanyModel] = await self.uow.company.get_by_query_all(
            **kwargs
        )
    ...
    async def update_company_by_id(
        self, company_id: UUID4, company_data: dict
    ) -> CompanyDB:
        ...
        self._check_company_exists(company=company)
        updated_company: CompanyModel = await self.uow.company.update_one_by_id(
            _id=company_id, **company_data
        )
    ...
    def _check_correct_invite_code(code: int | None) -> None:
        if not code:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Invite invalid"
            )
    ...
    def _check_invite_exists(invite: InviteModel | None) -> None:
        if not invite:
            raise HTTPException(
                ...
                detail="Incorrect data has been entered",
            )
